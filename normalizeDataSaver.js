export default class NormalizeDataSaver {
	...
	run() {
		...
		normalized.cleanerCount = this.#functions.clampInt(source.cleanerCount, 0, Number.MAX_SAFE_INTEGER, fallback.cleanerCount);
		normalized.cleaningMultiplier = this.#functions.clampInt(source.cleaningMultiplier, 1, 10, fallback.cleaningMultiplier);
		...
		normalized.aquariumCapacity = this.#functions.clampInt(source.aquariumCapacity, 0, 40, fallback.aquariumCapacity);
		...
		normalized.aquariumFishCounts = aquariumFishCounts.map(count => {
			count = this.#functions.clampInt(count, 0, fishLeft, 0);
			fishLeft -= count;
			return count
		});
		let foodFish;
